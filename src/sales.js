...
const TAX_RATE = 0.10;
// 人件費を除いた固定費163万円。限界利益がこれを超えれば損益分岐、営業利益 = 限界利益 − 163万円
const MONTHLY_FIXED_COST_EX_TAX = 1630000;
...
const OPERATING_BASE_PROFIT_NOTE =
  "※スプレッドシート入力の営業ベース利益は、固定費・税金・借入返済等を含む最終利益ではありません。";
...
/** PC表示時の売上管理ページ最大幅 */
const SALES_PAGE_MAX_WIDTH_PC = 1560;
...
function analysisCardWrap(variant, narrow) {
  ...
  return {
    ...
    overflow: "hidden",
    ...(narrow ? {} : { padding: "1.28rem 1.38rem" }),
  };
}
...
function analysisSecTitle(variant, marginBottom, narrow = false) {
  const v = ANALYSIS_CARD[variant] || ANALYSIS_CARD.trend;
...
function salesPageShellStyle(narrow, mobile) {
  return {
    padding: narrow ? (mobile ? "1rem .65rem" : "1.25rem .85rem") : "1.5rem 2.5rem",
    maxWidth: narrow ? "100%" : `min(${SALES_PAGE_MAX_WIDTH_PC}px, calc(100vw - 3rem))`,
    width: "100%",
    margin: "0 auto",
    boxSizing: "border-box",
    overflowX: "hidden",
...
function rGridCols(narrow, minPx = 170) {
  ...
  return `repeat(auto-fit, minmax(min(${minPx + 30}px, 100%), 1fr))`;
}
...
const OPERATING_PROFIT_LINE_DEFINITIONS = [
  { key: "strong", minOperatingProfit: OPERATING_PROFIT_STRONG_THRESHOLD, label: "強い月", badge: "強い" },
  { key: "good", minOperatingProfit: OPERATING_PROFIT_GOOD_THRESHOLD, label: "かなり良い", badge: "良い" },
  { key: "stable", minOperatingProfit: OPERATING_PROFIT_STABLE_THRESHOLD, label: "安定ライン", badge: "安定" },
  { key: "smallProfit", minOperatingProfit: 1, label: "黒字ライン", badge: "最低" },
];
function monthCostSnapshot_(m) {
  if (m == null) return {};
  if (typeof m === "number" || typeof m === "string") return { totalSalesSum: m };
  return {
    totalSalesSum: m.totalSalesSum,
    drinkPurchaseSum: m.drinkPurchaseSum,
    foodPurchaseSum: m.foodPurchaseSum,
    expenseSum: m.expenseSum,
    laborCostSum: m.laborCostSum,
    hasMonthlyCostSummary: m.hasMonthlyCostSummary,
  };
}
function resolveBreakEvenLine_(marginalProfit, operatingProfit, totalSales) {
  const sales = Number(totalSales || 0);
  const marginal = marginalProfit != null && Number.isFinite(Number(marginalProfit)) ? Number(marginalProfit) : null;
  if (sales <= 0 || marginal == null) {
    return {
      totalSales: sales > 0 ? sales : null,
      ...
      breakEvenMarginalTarget: MONTHLY_FIXED_COST_EX_TAX,
    };
  }
  const operating =
    operatingProfit != null && Number.isFinite(Number(operatingProfit))
      ? Number(operatingProfit)
      : marginal - MONTHLY_FIXED_COST_EX_TAX;
  const gap = marginal - MONTHLY_FIXED_COST_EX_TAX;
  const isAboveBreakEven = marginal >= MONTHLY_FIXED_COST_EX_TAX;
  let tier = { key: "below", label: "損益分岐未達", badge: "未達" };
  if (isAboveBreakEven) {
    tier = { key: "minimum", label: "損益分岐到達", badge: "最低" };
    for (const def of OPERATING_PROFIT_LINE_DEFINITIONS) {
      if (operating >= def.minOperatingProfit) {
        tier = def;
        break;
      }
    }
  ...
  return {
    ...
    gapLabel:
      gap >= 0
        ? `損益分岐超過：${formatSignedExTaxYen_(gap)}`
        : `損益分岐まで：${formatExTaxYen_(Math.abs(gap))}`,
    isAboveBreakEven,
    isAboveSafeLine: isAboveBreakEven,
    hasActualSales: true,
    breakEvenMarginalTarget: MONTHLY_FIXED_COST_EX_TAX,
  };
}
function buildBreakEvenAnalysis_(monthSnapshot) {
  const snap = monthCostSnapshot_(monthSnapshot);
  const marginal = calcMarginalProfitFromMonth_(snap);
  const operating = resolveOperatingProfit_(snap);
  return resolveBreakEvenLine_(marginal.profit, operating.profit, snap.totalSalesSum);
}
...
function countBreakEvenMonths_(months) {
  ...
  const countTier = (key) =>
    actualMonths.filter((m) => {
      ...
      if (key === "aboveBreakEven") return be?.isAboveBreakEven;
      if (key === "safeLine") return be?.tierKey !== "below";
      if (key === "stable") return ["stable", "good", "strong"].includes(be?.tierKey);
      if (key === "good") return ["good", "strong"].includes(be?.tierKey);
      if (key === "strong") return be?.tierKey === "strong";
      return false;
    }).length;
  return {
...
/** 限界利益 = 売上 − ドリンク仕入 − フード仕入 − 経費 − 人件費 */
function calcMarginalProfit_(totalSales, drinkPurchase, foodPurchase, expense, laborCost) {
  const sales = Number(totalSales || 0);
  if (!(sales > 0)) return { profit: null, rate: null };
  const profit =
    sales -
    Number(drinkPurchase || 0) -
    Number(foodPurchase || 0) -
    Number(expense || 0) -
    Number(laborCost || 0);
  return { profit, rate: calcRate(profit, sales) };
}
function calcMarginalProfitFromMonth_(m) {
  return calcMarginalProfit_(
    m?.totalSalesSum,
    m?.drinkPurchaseSum,
    m?.foodPurchaseSum,
    m?.expenseSum,
...
function resolveOperatingProfit_(m) {
  ...
  if (!(sales > 0) || marginal.profit == null) return { profit: null, rate: null };
  const profit = marginal.profit - MONTHLY_FIXED_COST_EX_TAX;
  return { profit, rate: calcRate(profit, sales) };
}
function formatReviewProfitYen_(profit) {
  if (profit == null || !Number.isFinite(Number(profit))) return "—";
  return yen(Math.round(Number(profit)));
}
function formatReviewProfitRate_(rate) {
  return pct1(rate);
}
...
function buildLandingForecast_(monthRows, yearlyTotalSales, targetMetrics) {
  ...
  const avgMonthlySales =
    performanceMonthCount > 0 ? Math.round(performanceSalesSum / performanceMonthCount) : null;
  ...
  const requiredMonthly =
    ...
    remainingNeeded > 0 &&
    remainingMonths > 0
      ? Math.round(remainingNeeded / remainingMonths)
      : null;
  const forecastGap =
  ...
  const avgMonthlySalesFromYearlyTotal =
    performanceMonthCount > 0 ? Math.round(yearlyTotalSales / performanceMonthCount) : null;
  return {
    hasFullYearTarget,
    enteredTargetSum,
...
function signedMetricColor_(value) {
  if (value == null || !Number.isFinite(Number(value))) return undefined;
  if (Number(value) === 0) return "rgba(240,232,208,0.94)";
  return Number(value) > 0 ? "#9ec9a8" : "#dca06a";
}
function formatSignedCustomerCountDiff_(diff) {
  if (diff == null || !Number.isFinite(Number(diff))) return "—";
  const n = Math.round(Number(diff));
  const sign = n > 0 ? "+" : "";
  return `${sign}${num(n)}名`;
}
function formatSignedUnitYenDiff_(diff) {
  if (diff == null || !Number.isFinite(Number(diff))) return "—";
  const n = Math.round(Number(diff));
  if (n === 0) return yen(0);
  const sign = n > 0 ? "+" : "-";
...
function flattenYearMonthRecords_(yearlyMonthData) {
  ...
  for (const item of yearlyMonthData || []) {
    if (!item.ok) continue;
    for (const r of item.records || []) out.push(r);
  }
  return out;
}
function findMaxActualBusinessDateInYear_(yearlyMonthData, targetYear) {
  ...
  for (const item of yearlyMonthData || []) {
    ...
    for (const r of item.records || []) {
      if (r?.metrics?.totalSales == null) continue;
      const bd = r.businessDate;
      if (!bd || !bd.startsWith(prefix)) continue;
      if (!maxDate || bd > maxDate) maxDate = bd;
    }
  }
  return maxDate;
}
function shiftDateYear_(dateStr, deltaYears) {
  if (!dateStr) return null;
  const [y, m, d] = dateStr.split("-").map(Number);
  return `${y + deltaYears}-${String(m).padStart(2, "0")}-${String(d).padStart(2, "0")}`;
}
function sumMetricsFromRecords_(records, opts = {}) {
  const { startDateInclusive, endDateInclusive, yearPrefix } = opts;
  let rows = (records || []).filter((r) => r?.metrics?.totalSales != null);
  if (yearPrefix) rows = rows.filter((r) => (r.businessDate || "").startsWith(yearPrefix));
  if (startDateInclusive) rows = rows.filter((r) => (r.businessDate || "") >= startDateInclusive);
  if (endDateInclusive) rows = rows.filter((r) => (r.businessDate || "") <= endDateInclusive);
...
function sumMonthRowMetrics_(months) {
  ...
  const customerCount = (months || []).reduce((s, m) => s + Number(m.customerCountSum || 0), 0);
  const drinkSales = (months || []).reduce((s, m) => s + Number(m.drinkSalesSum || 0), 0);
  const foodSales = (months || []).reduce((s, m) => s + Number(m.foodSalesSum || 0), 0);
  return { totalSales, customerCount, drinkSales, foodSales };
}
function sumPriorYearSalesThroughMonth_(endMonthNum, lastMonthProrate = 1) {
  let sum = 0;
  for (let mo = 1; mo < endMonthNum; mo++) {
    sum += PREVIOUS_YEAR_SALES_2025_MAP[mo] || 0;
  }
  if (endMonthNum >= 1 && endMonthNum <= 12) {
    sum += (PREVIOUS_YEAR_SALES_2025_MAP[endMonthNum] || 0) * lastMonthProrate;
  }
  return sum;
}
function sumPriorYearSalesThroughDate_(endDateStr) {
  if (!endDateStr) return 0;
  const endMonth = Number(endDateStr.slice(5, 7));
  const endDay = Number(endDateStr.slice(8, 10));
  const priorYear = Number(endDateStr.slice(0, 4)) - 1;
  const daysInMonth = new Date(priorYear, endMonth, 0).getDate();
  const prorate = daysInMonth > 0 ? endDay / daysInMonth : 1;
  return sumPriorYearSalesThroughMonth_(endMonth, prorate);
}
function buildMonthRowsFromYearData_(yearlyMonthData, year, currentBusinessDate, endMonthNum) {
  const rows = [];
  for (const item of yearlyMonthData || []) {
    if (!item.ok) continue;
    if (!String(item.month || "").startsWith(`${year}-`)) continue;
    const mn = Number(String(item.month).slice(5, 7));
    if (!Number.isFinite(mn) || mn > endMonthNum) continue;
    rows.push(aggregateMonthFromRecords_(item.records, item.month, currentBusinessDate, item.monthlySummary));
  }
  return rows.filter((m) => m.status === "集計済み" && Number(m.totalSalesSum || 0) > 0);
}
function buildYearlySamePeriodComparison_(monthRows, yearlyMonthData, priorYearMonthData, currentBusinessDate, targetYear) {
  const year = Number(targetYear) || new Date().getFullYear();
  const priorYear = year - 1;
  const aggregatedMonths = (monthRows || []).filter((m) => m.status === "集計済み" && Number(m.totalSalesSum || 0) > 0);
  if (!aggregatedMonths.length) return null;

  const sortedMonths = [...aggregatedMonths].sort((a, b) => a.targetMonth.localeCompare(b.targetMonth));
...
function enhanceYearlyMonthRowsForCharts_(monthRows) {
  return (monthRows || []).map((m) => {
    ...
    return {
      ...
      reviewOperatingProfitSum: operating.profit,
      reviewOperatingProfitRate: operating.rate,
      reviewOperatingProfitAbs: operating.profit != null ? Math.abs(operating.profit) : null,
...
function isMonthNegativeOperatingProfit_(m) {
...
function emptyMonthAggregate_(targetMonth, status, fetchError) {
  return {
    ...
    marginalProfitSum: null,
    marginalProfitRate: null,
    reviewOperatingProfitSum: null,
    reviewOperatingProfitRate: null,
  };
}
function aggregateMonthFromRecords_(records, targetMonth, currentBusinessDate, monthlySummary) {
  ...
  const marginal = calcMarginalProfitFromMonth_(costSnap);
  const reviewOperating = resolveOperatingProfit_(costSnap);
  return {
    targetMonth,
    monthLabel: monthLabelFromTarget_(targetMonth),
    status,
    ...
    breakEvenAnalysis: buildBreakEvenAnalysis_(costSnap),
    marginalProfitSum: marginal.profit,
    marginalProfitRate: marginal.rate,
...
function YearlyRankList({ title, variant, items, valueLabel, formatValue, narrow = false }) {
  ...
  return (
    <div style={{ ...analysisCard(variant), width: "100%", minWidth: 0, boxSizing: "border-box", ...(narrow ? {} : { padding: "1.15rem 1.25rem" }) }}>
      ...
      {items.length === 0 ? (
        <div style={{ fontSize: narrow ? ".78rem" : ".84rem", color: "rgba(240,232,208,0.45)" }}>データなし</div>
      ) : (
        items.map((r, i) => (
          <div key={r.targetMonth} style={{ padding: ".32rem 0", borderBottom: `1px solid ${analysisRowBorder(variant)}` }}>
            <div style={{ fontSize: rowLabelSize, color: "rgba(240,232,208,0.55)" }}>{i + 1}. {r.monthLabel}</div>
            <div style={{ fontSize: narrow ? ".82rem" : ".88rem", color: "rgba(240,232,208,0.82)" }}>
              <strong style={{ fontSize: rowValueSize, fontWeight: 600, color: "rgba(240,232,208,0.92)", fontFamily: SALES_NUMBER_FONT_FAMILY, ...SALES_NUMBER_TABULAR }}>{formatValue(r)}</strong>
              <span style={{ marginLeft: ".35rem", fontSize: rowMetaSize, color: "rgba(240,232,208,0.5)" }}>{valueLabel}</span>
            </div>
          </div>
        ))
      )}
  ...
  return (
    <div style={{ display: "grid", gap: ".55rem", minWidth: 0 }}>
      <div style={{ display: "grid", gridTemplateColumns: colGrid, gap: ".55rem" }}>
        <YearlySummaryBlock title="A. 今年の進捗" narrow={narrow} accent>
          <YearlySummaryMetricLine narrow={narrow} label="年間売上" value={dy(a.yearlyTotalSales)} emphasize />
          <YearlySummaryMetricLine
            narrow={narrow}
            label={a.hasFullYearTarget ? "年間目標" : "設定済み目標の合計"}
            value={dy(a.hasFullYearTarget ? a.fullYearTargetSum : a.enteredTargetSum)}
          />
          <YearlySummaryMetricLine narrow={narrow} label={progressLabel} value={pct(a.yearlyProgressRate)} strong />
          <YearlySummaryMetricLine
            narrow={narrow}
            label="実績月数 / 目標設定済み月数"
        ...
        <YearlySummaryBlock title="C. 年間着地見込み" narrow={narrow}>
          ...
          {landing?.hasFullYearTarget ? (
            <YearlySummaryMetricLine
              ...
              valueStyle={{
                color: landing?.forecastGap != null ? signedMetricColor_(landing.forecastGap) : undefined,
              }}
            />
          ) : null}
          {landing?.paceForecast != null ? (
            <div style={{ fontSize: narrow ? "0.68rem" : "0.72rem", color: "rgba(240,232,208,0.5)", lineHeight: 1.45, marginTop: ".04rem" }}>
              ※このままのペースなら、年間{dy(landing.paceForecast)}の見込みです。
            </div>
          ) : null}
          <div style={{ fontSize: narrow ? "0.68rem" : "0.72rem", color: "rgba(240,232,208,0.5)", lineHeight: 1.45, marginTop: ".04rem" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: colGrid, gap: ".55rem" }}>
        <YearlySummaryBlock title="E. 利益" narrow={narrow}>
          ...
          <YearlySummaryMetricLine
            narrow={narrow}
            label="年間限界利益率"
            value={a.yearlyMarginalProfitRate != null ? pct1(a.yearlyMarginalProfitRate) : "—"}
            emphasize
          />
          <YearlySummaryMetricLine
            narrow={narrow}
            label="年間営業利益"
            value={a.yearlyReviewOperatingProfit != null ? dy(a.yearlyReviewOperatingProfit) : "—"}
            emphasize
          />
          <YearlySummaryMetricLine
            narrow={narrow}
            label="年間営業利益率"
            value={a.yearlyReviewOperatingProfitRate != null ? pct1(a.yearlyReviewOperatingProfitRate) : "—"}
            emphasize
          />
          <YearlySummaryMetricLine
            narrow={narrow}
          ...
          <YearlySummaryMetricLine
            narrow={narrow}
            label="営業利益100万円超えの月数"
            value={`${num(a.strongOperatingProfitMonthCount)}ヶ月`}
          />
          <div style={{ fontSize: narrow ? "0.68rem" : "0.72rem", color: "rgba(240,232,208,0.5)", lineHeight: 1.45, marginTop: ".04rem" }}>
            {MARGINAL_PROFIT_NOTE}
            <span style={{ display: "block", marginTop: ".08rem" }}>{OPERATING_PROFIT_NOTE}</span>
        ...
        <YearlySummaryBlock title="F. 集客・飲食" narrow={narrow}>
...
function BreakEvenMonthlySummaryBlock({ breakEvenAnalysis, reviewOperatingProfit, reviewOperatingProfitRate, narrow }) {
  const be = breakEvenAnalysis;
  if (!be) return null;
...
function SummarySubCard({ title, children, accent, narrow }) {
  return (
    <div
      style={{
        padding: narrow ? ".5rem .58rem" : ".62rem .72rem",
        borderRadius: 6,
        border: accent ? "1px solid rgba(201,168,76,0.24)" : "1px solid rgba(201,168,76,0.16)",
        background: accent ? "rgba(201,168,76,0.05)" : "rgba(0,0,0,0.12)",
        minWidth: 0,
    ...
    <div style={{ display: "grid", gap: ".55rem", minWidth: 0, maxWidth: "100%" }}>
      <div style={{ display: "grid", gridTemplateColumns: topGrid, gap: ".55rem", minWidth: 0, maxWidth: "100%" }}>
        ...
        <BreakEvenMonthlySummaryBlock
          ...
          reviewOperatingProfit={a.reviewOperatingProfitSum}
          reviewOperatingProfitRate={a.reviewOperatingProfitRate}
          narrow={narrow}
        />
      </div>
...
export default function SalesModule({ events = [], navigateBack }) {
  ...
  const [priorYearMonthData, setPriorYearMonthData] = useState([]);
  ...
  const monthlyAnalysis = useMemo(() => {
    ...
    const breakEvenAnalysis = buildBreakEvenAnalysis_({
      totalSalesSum,
      drinkPurchaseSum,
      foodPurchaseSum,
      expenseSum,
    ...
    const marginal = calcMarginalProfit_(totalSalesSum, drinkPurchaseSum, foodPurchaseSum, expenseSum, laborCostSum);
  ...
  const yearlyAnalysis = useMemo(() => {
    ...
    const samePeriodComparison = buildYearlySamePeriodComparison_(
      monthRows,
      yearlyMonthData,
      priorYearMonthData,
      currentBusinessDate,
    ...
    let yearlyMarginalProfit = 0;
    let yearlyReviewOperatingProfit = 0;
    for (const m of aggregatedMonths) {
      const marginal = calcMarginalProfitFromMonth_(m);
      const operating = resolveOperatingProfit_(m);
      if (marginal.profit != null) yearlyMarginalProfit += marginal.profit;
      if (operating.profit != null) yearlyReviewOperatingProfit += operating.profit;
    }
    const yearlyMarginalProfitRate =
      yearlyTotalSales > 0 ? (yearlyMarginalProfit / yearlyTotalSales) * 100 : null;
    const yearlyReviewOperatingProfitRate =
      yearlyTotalSales > 0 ? (yearlyReviewOperatingProfit / yearlyTotalSales) * 100 : null;
    const positiveOperatingProfitMonthCount = endedMonths.filter((m) => {
      const op = resolveOperatingProfit_(m);
      return op.profit != null && op.profit > 0;
    }).length;
    const strongOperatingProfitMonthCount = endedMonths.filter((m) => {
      const op = resolveOperatingProfit_(m);
      return op.profit != null && op.profit > OPERATING_PROFIT_STRONG_THRESHOLD;
    }).length;
    const negativeOperatingProfitMonthCount = endedMonths.filter(isMonthNegativeOperatingProfit_).length;
    ...
    const profitTop3 = topN(
      ...
      (a, b) => Number(resolveOperatingProfit_(b).profit || 0) - Number(resolveOperatingProfit_(a).profit || 0),
      3
    ).map((m) => ({
      ...m,
      reviewOperatingProfitSum: resolveOperatingProfit_(m).profit,
      reviewOperatingProfitRate: resolveOperatingProfit_(m).rate,
    }));
    ...
    return {
      ...
      yearlyMarginalProfit: aggregatedMonths.length > 0 ? yearlyMarginalProfit : null,
      yearlyMarginalProfitRate,
      yearlyReviewOperatingProfit: aggregatedMonths.length > 0 ? yearlyReviewOperatingProfit : null,
      yearlyReviewOperatingProfitRate,
      yearlyFoodDrink,
      yearlyDrink,
      yearlyFood,
      yearlyVenueFee,
      ...
      strongOperatingProfitMonthCount,
      negativeOperatingCurrentMonths,
  ...
  return (
    <div
      style={salesPageShellStyle(vp.narrow, vp.mobile)}
      className="hb-view"
    >
      <div
        style={{
      ...
      {!loading && !error && rows.length > 0 && roleMode === "admin" && adminTab === "analysis" && (
        <div style={{ display:"grid", gap: vp.narrow ? ".75rem" : ".85rem", marginBottom:".75rem", ...analysisSectionWrap(vp.narrow) }}>
          <div style={analysisCardWrap("summary", vp.narrow)}>
            <div style={analysisSecTitle("summary", ".55rem", vp.narrow)}>月次サマリー</div>
            <MonthlySummaryPanel
          ...
          <div style={analysisCardWrap("insight", vp.narrow)}>
            <div style={analysisSecTitle("insight", ".5rem", vp.narrow)}>月次分析コメント</div>
            {monthlyImprovementComments.length === 0 ? (
              <div style={{ fontSize: vp.narrow ? ".78rem" : ".86rem", color: "rgba(240,232,208,0.5)", lineHeight: 1.55 }}>
          ...
          <div style={analysisCardWrap("composition", vp.narrow)}>
            <div style={analysisSecTitle("composition", ".5rem", vp.narrow)}>売上構成</div>
          ...
          <div style={analysisCardWrap("costProfit", vp.narrow)}>
            <div style={analysisSecTitle("costProfit", ".35rem", vp.narrow)}>コスト・利益比較（暫定）</div>
          ...
          <div style={analysisCardWrap("trend", vp.narrow)}>
            <div style={analysisSecTitle("trend", ".5rem", vp.narrow)}>日別売上推移</div>
          ...
          <div style={{ display:"grid", gridTemplateColumns: rGridCols(vp.narrow, 280), gap:".65rem" }}>
            <div style={{ ...analysisCardWrap("rankSales", vp.narrow), width: "100%", minWidth: 0, boxSizing: "border-box" }}>
              <div style={analysisSecTitle("rankSales", ".5rem", vp.narrow)}>月間売上TOP5</div>
            ...
            <div style={{ ...analysisCardWrap("rankUnder", vp.narrow), width: "100%", minWidth: 0, boxSizing: "border-box" }}>
              <div style={analysisSecTitle("rankUnder", ".5rem", vp.narrow)}>月間目標未達ワースト5</div>
          ...
          <div style={{ display:"grid", gridTemplateColumns: rGridCols(vp.narrow, 260), gap:".65rem" }}>
            <div style={{ ...analysisCardWrap("rankFoodDrink", vp.narrow), width: "100%", minWidth: 0, boxSizing: "border-box" }}>
              <div style={analysisSecTitle("rankFoodDrink", ".5rem", vp.narrow)}>月間飲食売上TOP10</div>
            ...
            <div style={{ ...analysisCardWrap("rankDrink", vp.narrow), width: "100%", minWidth: 0, boxSizing: "border-box" }}>
              <div style={analysisSecTitle("rankDrink", ".5rem", vp.narrow)}>月間ドリンク売上TOP10</div>
            ...
            <div style={{ ...analysisCardWrap("rankFood", vp.narrow), width: "100%", minWidth: 0, boxSizing: "border-box" }}>
              <div style={analysisSecTitle("rankFood", ".5rem", vp.narrow)}>月間フード売上TOP10</div>
        ...
        <div style={{ display: "grid", gap: vp.narrow ? ".75rem" : ".85rem", marginBottom: ".75rem", ...analysisSectionWrap(vp.narrow) }}>
          ...
          {!yearlyLoading && yearlyAnalysis && (
            <>
              ...
              <div style={analysisCardWrap("composition", vp.narrow)}>
                <div style={analysisSecTitle("composition", ".5rem", vp.narrow)}>月別経営レビュー</div>
                <div style={{ fontSize: vp.narrow ? "0.78rem" : "0.84rem", color: "rgba(240,232,208,0.58)", marginBottom: ".45rem" }}>
                  月をクリックすると月次分析へ移動します
                </div>
                {vp.narrow ? (
              ...
              <div style={analysisCardWrap("composition", vp.narrow)}>
                <div style={analysisSecTitle("composition", ".5rem", vp.narrow)}>仕入・原価率（参考）</div>
                {vp.narrow ? (
              ...
              <div style={analysisCardWrap("composition", vp.narrow)}>
                <div style={analysisSecTitle("composition", ".5rem", vp.narrow)}>月別前年比較</div>
                <div style={{ fontSize: vp.narrow ? ".62rem" : ".68rem", color: "rgba(240,232,208,0.5)", marginBottom: ".35rem" }}>月をクリックすると月次分析へ移動します</div>
                {vp.narrow ? (
              ...
              <div style={analysisCardWrap("trend", vp.narrow)}>
                <div style={analysisSecTitle("trend", ".5rem", vp.narrow)}>年次グラフ</div>
                <div style={{ fontSize: vp.narrow ? "0.78rem" : "0.84rem", color: "rgba(240,232,208,0.55)", marginBottom: ".45rem" }}>
                  グラフの月をクリックすると月次分析へ移動します
                </div>
                <YearlyYoYBarChart rows={yearlyAnalysis.monthlyYoYRows} onMonthClick={navigateToMonthAnalysis} />

                <div style={{ display: "grid", gridTemplateColumns: yearlyMonthChartGridCols_(vp.narrow), gap: vp.narrow ? ".75rem" : ".9rem", width: "100%", maxWidth: "100%", minWidth: 0, marginTop: ".75rem" }}>
                  <YearlyMonthBarChart narrow={vp.narrow} tall title="月別売上推移" rows={yearlyAnalysis.chartMonthRows} valueKey="totalSalesSum" barTone="linear-gradient(180deg, rgba(201,168,76,0.95), rgba(201,168,76,0.55))" taxMode={taxMode} onMonthClick={navigateToMonthAnalysis} />
                  <YearlyMonthBarChart
                    narrow={vp.narrow}
                    tall
                    title="月別 損益分岐との差"
                  ...
                  <YearlyMonthBarChart
                    ...
                    title="月別 営業利益"
                    rows={yearlyAnalysis.chartMonthRows}
                    valueKey="reviewOperatingProfitAbs"
                    barTone="linear-gradient(180deg, rgba(126,200,126,0.95), rgba(126,200,126,0.55))"
                    formatTop={(r) => formatReviewProfitYen_(r.reviewOperatingProfitSum)}
                    onMonthClick={navigateToMonthAnalysis}
                  />
                  <YearlyMonthBarChart
                    narrow={vp.narrow}
              ...
              <div style={analysisCardWrap("trend", vp.narrow)}>
                <div style={analysisSecTitle("trend", ".5rem", vp.narrow)}>ランキング</div>
                <div style={{ display: "grid", gridTemplateColumns: rGridCols(vp.narrow, 220), gap: ".65rem" }}>
                  <YearlyRankList narrow={vp.narrow} title="売上が強い月" variant="rankSales" items={yearlyAnalysis.salesTop3} valueLabel="売上" formatValue={(r) => dy(r.totalSalesSum)} />
                  <YearlyRankList narrow={vp.narrow} title="目標との差が大きい月" variant="rankUnder" items={yearlyAnalysis.underWorst3} valueLabel="達成率" formatValue={(r) => pct(r.progressRate)} />
                  <YearlyRankList narrow={vp.narrow} title="飲食が強い月" variant="rankFoodDrink" items={yearlyAnalysis.foodDrinkTop3} valueLabel="飲食売上" formatValue={(r) => dy(r.foodDrinkSalesIncludingBandSum)} />
                  <YearlyRankList narrow={vp.narrow} title="ドリンクが強い月" variant="rankDrink" items={yearlyAnalysis.drinkTop3} valueLabel="ドリンク売上" formatValue={(r) => dy(r.drinkSalesSum)} />
                  <YearlyRankList narrow={vp.narrow} title="フードが強い月" variant="rankFood" items={yearlyAnalysis.foodTop3} valueLabel="フード売上" formatValue={(r) => dy(r.foodSalesSum)} />
                  <YearlyRankList narrow={vp.narrow} title="営業利益が強い月" variant="rankSales" items={yearlyAnalysis.profitTop3} valueLabel="営業利益" formatValue={(r) => dy(r.reviewOperatingProfitSum)} />
                  <YearlyRankList narrow={vp.narrow} title="飲食単価が高い月" variant="rankFoodDrink" items={yearlyAnalysis.foodDrinkUnitPriceTop3} valueLabel="飲食単価" formatValue={(r) => formatUnitYen_(r.foodDrinkUnitPrice)} />
                  <YearlyRankList narrow={vp.narrow} title="集客が多い月" variant="rankDrink" items={yearlyAnalysis.customerCountTop3} valueLabel="集客人数" formatValue={(r) => formatCustomerCountLabel_(r.customerCountSum)} />
                  <YearlyRankList narrow={vp.narrow} title="営業ベース利益率が高い月" variant="rankFood" items={yearlyAnalysis.operatingProfitRateTop3} valueLabel="営業ベース利益率" formatValue={(r) => pct1(r.operatingProfitRate)} />
                </div>
              </div>

              <div style={analysisCardWrap("costProfit", vp.narrow)}>
                <div style={analysisSecTitle("costProfit", ".35rem", vp.narrow)}>年間コスト状況（暫定）</div>
                <div style={{ ...analysisNote({}, vp.narrow), marginBottom: ".45rem" }}>
                  ※人件費は翌月まとめて反映されます。仕入・経費は月末に売掛分が加算されるため、月中は暫定値です。
                ...
                <div style={{ marginTop: ".55rem", paddingTop: ".5rem", borderTop: "1px dashed rgba(201,168,76,0.2)" }}>
                  <div style={{ fontSize: ".66rem", color: "rgba(201,168,76,0.85)", marginBottom: ".32rem" }}>営業粗利</div>
                  <div style={{ display: "grid", gridTemplateColumns: rGridCols(vp.narrow, 170), gap: ".32rem .55rem", fontSize: vp.narrow ? ".88rem" : ".86rem", color: "rgba(240,232,208,0.85)" }}>
                    <div>
                      営業粗利{" "}
                      <span style={{ color: "#f0e8d0" }}>
