...
説明文のみを出力し、前置きや後書きは不要です。`;
  ...
  // 貸切判定 + お客様名抽出
  const isRentalEvent = (name) => /貸切|貸し切り/.test(name||"");
  const extractCustomerName = (name) => {
    if (!name) return "";
    let n = name;
    // 時間帯ラベルや [昼] [夜] [深夜] を除去
        ...
        for (const imp of imported) {
          ...
          if (isRentalEvent(imp.name)) {
            ...
            if (!dupRental && customerName) {
              ...
              // 時間帯ラベルを抽出（あれば希望時間に含める）
              const labelMatch = (imp.name || "").match(/[\[（(](昼|夜|深夜|朝|午前|午後)[\]）)]|^(昼|夜|深夜|朝|午前|午後)\s/);
      ...
      <div style={S.hdr} className="hb-hdr">
        <div style={{display:"flex",alignItems:"center",gap:".75rem"}}>
          ...
          <div style={S.logo} className="hb-logo" onClick={goHome} role="button" title="トップへ">
            HONEY BEE <small style={S.logoSm}>{view==="home"?"Operation":view.startsWith("events")?"Event Manager":view==="rentals"?"Rentals":view==="settlement"?"Settlement":""}</small>
          </div>
        </div>
