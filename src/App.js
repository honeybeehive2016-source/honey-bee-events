...
const emptyForm = { name:"",date:"",day:"",open:"",start:"",price:"",cap:"",perf:"",desc:"",url:"",notes:"",genre:"",rehearsal:"",poster:"",timetable:"",reference:"",seatable:true,reserveNotes:"" };
...
説明文のみを出力し、前置きや後書きは不要です。`;
  ...
  return(
    <div style={S.app}>
      ...
      <div style={S.hdr} className="hb-hdr">
        <div style={{display:"flex",alignItems:"center",gap:".75rem"}}>
          ...
          <div style={S.logo} className="hb-logo" onClick={goHome} role="button" title="トップへ">
            HONEY BEE <small style={S.logoSm}>{view==="home"?"Operation":view.startsWith("events")?"Event Manager":view==="rentals"?"Rentals":view==="settlement"?"Settlement":view==="today"?"Today":view==="shift"?"Shifts":view==="reservation"?"Reservations":""}</small>
          </div>
        </div>
        ...
        <div style={{padding:"2.5rem 2rem",maxWidth:1100,margin:"0 auto"}} className="hb-view">
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:"1.25rem"}} className="hb-module-grid">
            {[
              ...
              {key:"reservation", icon:"📞", title:"予約管理", desc:"フォーム予約 / 電話予約 / 受付チェック", ready:true},
      ...
      {view==="reservation"&&(
        <ReservationModule events={events} shifts={shiftsList} navigateBack={navigateBack}/>
      )}
