...
説明文のみを出力し、前置きや後書きは不要です。`;
  ...
  const deleteEvent = async (i) => {
    ...
    try {
      const ev = events[i];
      if (ev && ev._id) await deleteDoc(doc(db, "events", ev._id));
    ...
    <div style={S.app}>
      ...
      <div style={S.hdr} className="hb-hdr">
        <div style={{display:"flex",alignItems:"center",gap:".75rem"}}>
          ...
          <div style={S.logo} className="hb-logo" onClick={goHome} role="button" title="トップへ" style={{cursor:"pointer",display:"flex",alignItems:"center",gap:".5rem"}}>
            <img src={process.env.PUBLIC_URL + "/honeybee_logo.png"} alt="HONEY BEE" style={{height:"32px",display:"block"}}/>
            <small style={S.logoSm}>{view==="home"?"Operation":view.startsWith("events")?"Event Manager":""}</small>
          </div>
        </div>
        ...
        <div style={{padding:"2.5rem 2rem",maxWidth:1100,margin:"0 auto"}} className="hb-view">
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:"1.25rem"}} className="hb-module-grid">
            {[
              ...
              {key:"rentals", icon:"🍽", title:"貸切管理", desc:"問い合わせから成約までの一元管理 / 返信文AI", ready:false},
              {key:"settlement", icon:"💰", title:"アーティスト精算", desc:"チャージバック計算・精算メモ生成", ready:false},
              {key:"checklist", icon:"✅", title:"業務チェックリスト", desc:"開店前 / イベント中 / 終演後 / 閉店", ready:false},
              {key:"today", icon:"📅", title:"本日の営業", desc:"今日のイベント情報を1画面で確認", ready:false},
            ].map(m => (
