...
説明文のみを出力し、前置きや後書きは不要です。`;
  ...
  const handleDeduplicateEvents = async () => {
    if (!window.confirm("同じ日付・同じイベント名のイベントが複数ある場合、最新保存のもの以外を削除します。\nよろしいですか？")) return;
    const groups = {};
    events.forEach(ev => {
    ...
    if (toDelete.length === 0) { alert("重複イベントは見つかりませんでした。"); return; }
    try {
      await Promise.all(toDelete.map(id => deleteDoc(doc(db, "events", id))));
