...
import KitchenNotesModule from "./kitchenNotes";
...
const emptyForm = { name:"",date:"",day:"",open:"",start:"",price:"",cap:"",perf:"",desc:"",url:"",notes:"",genre:"",rehearsal:"",poster:"",timetable:"",reference:"",seatable:true,reserveNotes:"",noBooking:false,bookingStatus:"pending_detail",photoOk:"unset",smokeOk:"unset",images:[],galleryNote:"",remark:"" };

function bookingStatusLabel(ev) {
  const eff = effectiveBookingStatus(ev);
  return BOOKING_STATUS_OPTIONS.find(o => o.value === eff)?.label || eff;
}
function bookingStatusCalStyle(ev) {
  const eff = effectiveBookingStatus(ev);
  switch (eff) {
    case "pending_detail":
      return { borderLeft: "2px solid #f4a261", background: "rgba(244,162,97,0.14)" };
    case "pending_booking":
      return { borderLeft: "2px solid #c9a84c", background: "rgba(201,168,76,0.12)" };
    case "closed":
      return { borderLeft: "2px solid #666666", background: "rgba(102,102,102,0.18)" };
    default:
      return { borderLeft: "2px solid #7ec87e", background: "rgba(126,200,126,0.12)" };
  }
}
  ...
  return(
    <div>
      ...
      <div style={{display:"flex",justifyContent:"center",gap:".75rem",marginBottom:".5rem",fontSize:".58rem",color:"rgba(240,232,208,0.45)",flexWrap:"wrap"}}>
        <span style={{display:"flex",alignItems:"center",gap:".25rem"}}><span style={{display:"inline-block",width:8,height:10,background:"rgba(244,162,97,0.14)",borderLeft:"2px solid #f4a261",borderRadius:1}}/>詳細確認</span>
        <span style={{display:"flex",alignItems:"center",gap:".25rem"}}><span style={{display:"inline-block",width:8,height:10,background:"rgba(201,168,76,0.12)",borderLeft:"2px solid #c9a84c",borderRadius:1}}/>受付前</span>
        <span style={{display:"flex",alignItems:"center",gap:".25rem"}}><span style={{display:"inline-block",width:8,height:10,background:"rgba(126,200,126,0.12)",borderLeft:"2px solid #7ec87e",borderRadius:1}}/>受付中</span>
        <span style={{display:"flex",alignItems:"center",gap:".25rem"}}><span style={{display:"inline-block",width:8,height:10,background:"rgba(102,102,102,0.18)",borderLeft:"2px solid #666",borderRadius:1}}/>終了</span>
        <span style={{display:"flex",alignItems:"center",gap:".25rem"}}><span style={{display:"inline-block",width:8,height:10,background:"rgba(126,200,227,0.18)",borderLeft:"2px solid #7ec8e3",borderRadius:1}}/>貸切</span>
      </div>
                ...
                return (
                  <div key={ei}
                    ...
                    style={{
                      ...
                      ...(isRental
                        ? { background:"rgba(126,200,227,0.18)", borderLeft:"2px solid #7ec8e3" }
                        : calBooking),
                      borderRadius:2,cursor:"pointer",color:"#f0e8d0cc",overflow:"hidden",whiteSpace:"nowrap",textOverflow:"ellipsis"
                    }}
                    title={isRental?`貸切: ${it.contactName||""}（${it.purpose||""}）`:`${it.name} — ${bookingStatusLabel(it._orig)}`}>
                    {label}
                  </div>
                );
              })}
...
説明文のみを出力し、前置きや後書きは不要です。`;
  ...
  /** 編集中イベントを新規作成状態へ複製（別ドキュメントとして保存するまで上書きしない） */
  const duplicateEventAsNew = () => {
    let next;
    try {
      next = JSON.parse(JSON.stringify(form));
    } catch {
      next = { ...form };
    }
    const base = String(next.name || "").trim();
    next.name = base ? `${base}（コピー）` : "（コピー）";
    next.date = "";
    next.day = "";
    next.bookingStatus = "pending_detail";
    delete next._id;
    ...
    setOutputs(null);
    setAiError("");
    alert("新規イベントとして複製しました。日付と内容を確認して保存してください。");
  };
      ...
      try {
        ...
        for (const imp of imported) {
          ...
          if (existing && existing._id) {
            ...
            await setDoc(doc(db, "events", id), { ...imp, bookingStatus: "pending_detail" });
            added++;
          }
  ...
  return(
    <div style={S.app}>
      ...
      <style>{`
        ...
        .hb-form-2col { display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; align-items: start; }
      ...
      <div style={S.hdr} className="hb-hdr">
        <div style={{display:"flex",alignItems:"center",gap:".75rem"}}>
          ...
          <div style={S.logo} className="hb-logo" onClick={goHome} role="button" title="トップへ">
            HONEY BEE <small style={S.logoSm}>{view==="home"?"Operation":view.startsWith("events")?"Event Manager":view==="rentals"?"Rentals":view==="settlement"?"Settlement":view==="today"?"Today":view==="kitchen"?"Kitchen":view==="shift"?"Shifts":view==="reservation"?"Reservations":view==="seat_layout"?"Seats":""}</small>
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{display:"inline-flex",gap:".4rem",alignItems:"center"}}>
            {[
              ...
              {key:"kitchen", label:"🍳 厨房共有", views:["kitchen"]},
        ...
        <div style={{padding:"2.5rem 2rem",maxWidth:1100,margin:"0 auto"}} className="hb-view">
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:"1.25rem"}} className="hb-module-grid">
            {[
              ...
              {key:"kitchen", icon:"🍳", title:"厨房共有", desc:"仕込み・欠品・イベント準備の共有メモ", ready:true},
          ...
          {listMode==="list"&&(
            <>
              ...
              {sortedEvents.map((e,si)=>{
                ...
                const bs = effectiveBookingStatus(e);
                return(
                  <div key={si} style={S.card(false)}>
                    <div>
                      <div style={{fontFamily:"Georgia,serif",fontSize:".95rem",marginBottom:".28rem",display:"flex",alignItems:"center",gap:".5rem",flexWrap:"wrap"}}>
                        <span>{e.name||"（無題）"}</span>
                        <span style={{
                          fontSize:".58rem",letterSpacing:".06em",padding:".12rem .45rem",borderRadius:3,
                          background:bs==="pending_detail"?"rgba(244,162,97,0.18)":
                            bs==="pending_booking"?"rgba(201,168,76,0.15)":
                            bs==="closed"?"rgba(102,102,102,0.25)":
                            "rgba(126,200,126,0.15)",
                          color:bs==="closed"?"#aaa":"#f0e8d0",
                          border:`1px solid ${bs==="open"?"rgba(126,200,126,0.35)":
                            bs==="closed"?"#555":"rgba(201,168,76,0.25)"}`
                        }}>{bookingStatusLabel(e)}</span>
                      </div>
      ...
      {view==="events_form"&&(
        <div style={{padding:"1.5rem 2rem 7rem"}} className="hb-view">
            ...
            <div style={{padding:"1rem 1.1rem",background:"rgba(244,162,97,0.02)"}}>
              <div style={S.fgrid} className="hb-form-grid">
                ...
                <Field label="🎟 予約設定" full>
                  <div style={{...S.lbl,marginBottom:".35rem"}}>予約受付ステータス（お客様フォーム）</div>
                  <select
                    style={{...S.inp,marginBottom:".5rem"}}
                    value={BOOKING_STATUS_OPTIONS.some(o => o.value === form.bookingStatus) ? form.bookingStatus : effectiveBookingStatus(form)}
                  ...
                  <div style={{fontSize:".65rem",color:"rgba(240,232,208,0.45)",marginBottom:".65rem",lineHeight:1.5}}>
                    お客様向けフォームでは「予約受付中」のイベントのみ選択できます（未設定の既存データは「予約受付中」とみなされます）。「予約不可」にチェックを入れた場合は、この設定より優先されます。
                  </div>
          ...
          <div style={{position:"fixed",bottom:0,left:0,width:"100%",zIndex:60,background:"rgba(10,10,10,0.97)",borderTop:"1px solid rgba(201,168,76,0.25)",padding:".75rem 2rem",display:"flex",gap:".75rem",alignItems:"center",flexWrap:"wrap",backdropFilter:"blur(8px)"}}>
            ...
            {editingIdx!==null&&(
              <button type="button" style={{...S.btn("sm"),borderColor:"rgba(201,168,76,0.35)",color:"#c9a84c",padding:".45rem .9rem"}} onClick={duplicateEventAsNew}>📋 複製</button>
            )}
            {editingIdx!==null&&(
